...
function BDOMod_OnLoad()
	-- Dedicated event frame (keeps the orb frames purely visual/clickable)
	if not BDOMod_EventFrame then
		BDOMod_EventFrame = CreateFrame("Frame", "BDOMod_EventFrame", UIParent)
		BDOMod_EventFrame:SetScript("OnEvent", function(_, event, ...)
			BDOMod_OnEvent(event, ...)
		end)
	end

	if (BDOMod_EventFrame.UnregisterAllEvents) then
		BDOMod_EventFrame:UnregisterAllEvents()
	end

	BDOMod_EventFrame:RegisterEvent("UNIT_HEALTH_FREQUENT")
	BDOMod_EventFrame:RegisterEvent("UNIT_POWER_FREQUENT")
	BDOMod_EventFrame:RegisterEvent("UNIT_DISPLAYPOWER")
	BDOMod_EventFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
	BDOMod_EventFrame:RegisterEvent("SPELL_UPDATE_USABLE")
	BDOMod_EventFrame:RegisterEvent("ACTIONBAR_PAGE_CHANGED")
end
...
local function handleExpReputationBars()
	...
	if (MainMenuExpBar and MainMenuExpBar.IsVisible and MainMenuExpBar:IsVisible()) then
		if (ReputationWatchBar and ReputationWatchBar.IsVisible and ReputationWatchBar:IsVisible()) then
...
local function reconfigUI()
	...
	-- Action bar page number (frame name varies by client/version)
	if (MainMenuBarPageNumber) then
		MainMenuBarPageNumber:Hide()
		if (MainMenuBarPageNumber.SetScript) then
			MainMenuBarPageNumber:SetScript("OnShow", function(self) self:Hide() end)
		end
	elseif (MainMenuBarArtFrame and MainMenuBarArtFrame.PageNumber) then
		MainMenuBarArtFrame.PageNumber:Hide()
		if (MainMenuBarArtFrame.PageNumber.SetScript) then
			MainMenuBarArtFrame.PageNumber:SetScript("OnShow", function(self) self:Hide() end)
		end
	elseif (MainActionBar and MainActionBar.ActionBarPageNumber) then
		-- Anniversary/modern Classic: keep the buttons, hide only the number text
		local pageNumber = MainActionBar.ActionBarPageNumber
		if (pageNumber.Text) then
			pageNumber.Text:Hide()
			pageNumber.Text:SetAlpha(0)
		end
	end
	...
	-- Casting bar frame name varies by client/version (Anniversary uses PlayerCastingBarFrame)
	local castingBar = CastingBarFrame or PlayerCastingBarFrame
	if (castingBar) then
		castingBar:SetScale(scaleFactor*75/100)
	end
	...
	-- Anniversary uses GuildMicroButton ("Guild & Communities") instead of/alongside SocialsMicroButton
	if (SocialsMicroButton) then SocialsMicroButton:ClearAllPoints() end
	if (GuildMicroButton) then GuildMicroButton:ClearAllPoints() end
	WorldMapMicroButton:ClearAllPoints()
	MainMenuMicroButton:ClearAllPoints()
	HelpMicroButton:ClearAllPoints()
	
	MainMenuBarBackpackButton:ClearAllPoints()
	if (KeyRingButton) then KeyRingButton:ClearAllPoints() end
	CharacterBag0Slot:ClearAllPoints()
	CharacterBag1Slot:ClearAllPoints()
	CharacterBag2Slot:ClearAllPoints()
	CharacterBag3Slot:ClearAllPoints()
	if (HelpOpenWebTicketButton) then HelpOpenWebTicketButton:ClearAllPoints() end
	...
	if (SocialsMicroButton) then SocialsMicroButton:SetScale(scaleFactor*42/100) end
	if (GuildMicroButton) then GuildMicroButton:SetScale(scaleFactor*42/100) end
	...
	if (KeyRingButton) then KeyRingButton:SetScale(scaleFactor*37/100) end
	...
	if (HelpOpenWebTicketButton) then HelpOpenWebTicketButton:SetScale(scaleFactor*75/100) end
	
	if (HelpOpenWebTicketButton) then HelpOpenWebTicketButton:SetPoint("BOTTOM", UIParent, "BOTTOM",-140,-2) end
	-- Move the 8 micro buttons (Character -> Help) two slots to the right (+50 X)
	CharacterMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",-62,-2)
	SpellbookMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",-37,-2)
	TalentMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",-12,-2)
	QuestLogMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",13,-2)
	if (SocialsMicroButton) then SocialsMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",40,-2) end
	if (GuildMicroButton) then GuildMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",40,-2) end
	WorldMapMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",67,-2)
	MainMenuMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",92,-2)
	HelpMicroButton:SetPoint("BOTTOM", UIParent, "BOTTOM",117,-2)
	
	MainMenuBarBackpackButton:SetPoint("BOTTOM", UIParent, "BOTTOM",400,1)
	if (KeyRingButton) then KeyRingButton:SetPoint("BOTTOM", UIParent, "BOTTOM",217,0) end
	...
	-- Action bar paging buttons (names vary by client/version)
	local actionBarUpButton = ActionBarUpButton or (MainActionBar and MainActionBar.ActionBarPageNumber and MainActionBar.ActionBarPageNumber.UpButton)
	local actionBarDownButton = ActionBarDownButton or (MainActionBar and MainActionBar.ActionBarPageNumber and MainActionBar.ActionBarPageNumber.DownButton)
	...
	if (StanceButton2) then
		StanceButton2:ClearAllPoints()
		StanceButton2:SetScale(scaleFactor*48/100)
		StanceButton2:SetPoint("BOTTOM", UIParent, "BOTTOM",-286,1)
		StanceButton2.ignoreFramePositionManager = true
	end

	if (StanceButton3) then
		StanceButton3:ClearAllPoints()
		StanceButton3:SetScale(scaleFactor*48/100)
		StanceButton3:SetPoint("BOTTOM", UIParent, "BOTTOM",-256,1)
		StanceButton3.ignoreFramePositionManager = true
	end

	if (StanceButton4) then
		StanceButton4:ClearAllPoints()
		StanceButton4:SetScale(scaleFactor*48/100)
		StanceButton4:SetPoint("BOTTOM", UIParent, "BOTTOM",-226,1)
		StanceButton4.ignoreFramePositionManager = true
	end

	if (StanceButton5) then
		StanceButton5:ClearAllPoints()
		StanceButton5:SetScale(scaleFactor*48/100)
		StanceButton5:SetPoint("BOTTOM", UIParent, "BOTTOM",-196,1)
		StanceButton5.ignoreFramePositionManager = true
	end

	if (StanceButton6) then
		StanceButton6:ClearAllPoints()
		StanceButton6:SetScale(scaleFactor*48/100)
		StanceButton6:SetPoint("BOTTOM", UIParent, "BOTTOM",-166,1)
		StanceButton6.ignoreFramePositionManager = true
	end

	-- Additional stance buttons (Paladin auras include Crusader + Sanctity on Anniversary clients)
	if (StanceButton7) then
		StanceButton7:ClearAllPoints()
		StanceButton7:SetScale(scaleFactor*48/100)
		StanceButton7:SetPoint("BOTTOM", UIParent, "BOTTOM",-136,1)
		StanceButton7.ignoreFramePositionManager = true
	end

	if (StanceButton8) then
		StanceButton8:ClearAllPoints()
		StanceButton8:SetScale(scaleFactor*48/100)
		StanceButton8:SetPoint("BOTTOM", UIParent, "BOTTOM",-106,1)
...
local function setupOrbs()
	...
	-- Class color (CUSTOM_CLASS_COLORS if present, otherwise RAID_CLASS_COLORS)
	local _, classFile = UnitClass("player")
	local classColors = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)
	local color = classFile and classColors and classColors[classFile]
	if (color and color.r and color.g and color.b) then
		BDOMod_RedOrb:SetVertexColor(color.r, color.g, color.b)
	else
